...
class Curl_Requester implements Requester
{
    public $ch;
    ...
    public function options($options): void
    {
         ...
         * argument.
         */
        $possible_options = [CURLOPT_TIMEOUT => 'timeout', CURLOPT_CAINFO => 'ca', CURLOPT_USERAGENT => 'user_agent', CURLOPT_PROXY => 'proxy_url', CURLOPT_PROXYPORT => 'proxy_port'];
    ...
    public function execute($url, $method, $headers, $body = null): array
    {
        ...
        if ($result === false) {
             ...
             *     return FALSE).
             */
            $result = json_encode(['stat' => 'FAIL', 'code' => $errno, 'message' => $error]);
            $success = false;
        } else {

...
class Auth extends Client
{
    ...
    public function logo()
    {
        ...
        $params = [];
        return self::api_call($method, $endpoint, $params);
    }
    ...
    public function preauth($user_identifier, $ipaddr = null, $trusted_device_token = null, $username = true)
    {
    ...
    public function auth($user_identifier, $factor, $factor_params, $ipaddr = null, $async = false, $username = true, $timeout = 60)
    {
        assert(is_string($user_identifier));
        assert(is_string($factor) && in_array($factor, ['auto', 'push', 'passcode', 'sms', 'phone'], true));
        ...
        if (!$requester_timeout || $requester_timeout < $timeout) {
            self::set_requester_option('timeout', $timeout);
        }
        try {
            $result = self::json_api_call($method, $endpoint, $params);
        } finally {
            ...
            if ($requester_timeout) {
                self::set_requester_option('timeout', $requester_timeout);
            } else {

...
class File_Requester implements Requester
{
    public $http_options;
    public function __construct()
    {
        $this->http_options = ['http' => [
            /*
    ...
    protected static function parse_http_response_header($headers): int|false
    {
        /*
        ...
        $status_codes = array_map(function ($header) use ($status_code_regex): int {
    ...
    public function execute($url, $method, $headers, $body = null): array
    {
        ...
        if ($result === false) {
             ...
             *     return FALSE).
             */
            $result = json_encode(['stat' => 'FAIL', 'code' => $errno, 'message' => $message]);
            $success = false;
        } else {
        ...
        return ['response' => $result, 'success' => $success, 'http_status_code' => $http_status_code];
    }
}